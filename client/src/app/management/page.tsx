...
export default function ManagementPage() {
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">経営管理</h1>
        <p className="text-sm text-gray-500 mt-1">会社全体の状況を一画面で把握</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-gray-700">要対応事項</h2>
            <span className="text-xs text-red-500 font-medium">{criticalItems.length}件</span>
          </div>
          <div className="space-y-3">
            {criticalItems.map((item, idx) => (
              <div
                key={idx}
                className="flex items-start gap-3 p-3 rounded-xl hover:bg-white/60 transition-all cursor-pointer group"
              >
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-800 group-hover:text-primary-600 transition-colors">
                    {item.title}
                  </h4>
        ...
        <div className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-gray-700">部門別業務負荷</h2>
          </div>
              ...
              <div key={dept.name}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-400">
                    ...
                    {dept.overdue > 0 && (
                      <span className="text-red-500">{dept.overdue}件超過</span>
                    )}
                  </div>
                </div>
      ...
      <div className="glass rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-700">改善提案</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {recentImprovements.map((item, idx) => (
            <div
              key={idx}
              className="p-4 rounded-xl bg-white/50 hover:bg-white/70 transition-all cursor-pointer"
            >
              <div className="flex items-center gap-2 mb-2">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium ${
                  item.status === '完了' ? 'text-emerald-600 bg-emerald-50' :
                  item.status === '実行中' ? 'text-blue-600 bg-blue-50' :
              ...
              <p className="text-xs text-gray-400 mt-1">提案者: {item.proposer}</p>
